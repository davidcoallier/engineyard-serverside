  ...
  module Serverside
    class Deploy::Configuration
      ...
      def source_cache_strategy(shell)
        @strategy ||=
          if git
            load_strategy(EY::Serverside::Strategies::Git, shell, git)
          elsif archive
            load_strategy(EY::Serverside::Strategies::Archive, shell, archive)
          else # repo is allowed to be nil
            load_strategy(EY::Serverside::Strategies.const_get(strategy), shell, repo)
          end
      end
